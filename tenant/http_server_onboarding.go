...
func NewHTTPOnboardHandler(log *zap.Logger, userService influxdb.OnboardingService) *OnboardHandler {
	svr := &OnboardHandler{
		api:           kithttp.NewAPI(kithttp.WithLog(log)),
		log:           log,
		onboardingSvc: userService,
	}
	...
	r.Route("/", func(r chi.Router) {
		r.Post("/", svr.handleOnboardRequest)
...
// handleIsOnboarding is the HTTP handler for the GET /api/v2/setup route.
